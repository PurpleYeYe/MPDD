...
class FcClassifier(nn.Module):
    def __init__(self, input_dim, layers, output_dim, dropout=0.3, use_bn=False, opt=None):
        super().__init__()
        self.all_layers = []
        for i in range(0, len(layers)):
            if opt is not None and hasattr(opt, 'lowrank_rank'):
                rank = opt.lowrank_rank
                from models.networks.lowrank import LowRankLinear
                self.all_layers.append(LowRankLinear(input_dim, layers[i], rank=rank))
            else:
